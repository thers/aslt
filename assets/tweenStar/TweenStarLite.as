class tweenStar.TweenStarLite
{
  ...
  function TweenStarLite(_mytarget, _duration, _vars, _isTween, _realTarget)
  {
    ...
    this.ease = tweenStar.Ease.EaseTable[tweenStar.Ease.LINEAR];
    this.props = [];
    if(_isTween)
    {
      ...
      if(this.vars.ease != undefined)
      {
        ...
        {
          this.ease = tweenStar.Ease.EaseTable[this.vars.ease];
        }
        else
        {
    ...
    this.updateMC = (typeof _realTarget === "undefined" ? this.target : _realTarget).createEmptyMovieClip("TSLContainerMC",tweenStar.TweenStarLite.tweenMCDepth);
    this.updateMC.tweenStarUpdateRef = this;
    this.updateMC.onEnterFrame = function()
    {
      this.tweenStarUpdateRef.updateAll();
    };
    this.target[tweenStar.TweenStarLite.tweenStarLiteRef] = new tweenStar.TweenStarLiteRef(this.updateMC,this.props,this.vars);
  }
  ...
  {
    tweenStar.TweenStarLite.removeTweenOf(target);
    return new tweenStar.TweenStarLite(target,duration,vars,true);
  }
  
  static function toWithExplicitContainer(target, duration, vars, container)
  {
    tweenStar.TweenStarLite.endTweenOf(target, true);
    return new tweenStar.TweenStarLite(target, duration, vars, true, container);
  }
  
  static function delayCall(target, duration, vars)
  {
    tweenStar.TweenStarLite.removeTweenOf(target);
    return new tweenStar.TweenStarLite(target,duration,vars,false);
  }
  
  static function setDelay(target, _duration)
  {
    var _loc1_ = target[tweenStar.TweenStarLite.tweenStarLiteRef];
    _loc1_.updateMC.tweenStarUpdateRef.startTime = 0;
    _loc1_.updateMC.tweenStarUpdateRef.duration = _duration * tweenStar.TweenStarLite.frameRate;
  }
  ...
  {
    var instance = target[tweenStar.TweenStarLite.tweenStarLiteRef];
	
    if(instance != undefined)
    {
      for(var prop in instance.props)
      {
        target[instance.props[prop][0]] = instance.props[prop][2];
      }
	  
      if(instance.vars.onComplete && forceComplete)
      {
        instance.vars.onComplete.apply(instance.vars.onCompleteScope, instance.vars.onCompleteArgs);
      }
	  
      delete instance.updateMC.onEnterFrame;
	  
      instance.updateMC.removeMovieClip();
    }
  }
